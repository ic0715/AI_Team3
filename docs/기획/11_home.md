...
| 기능 | 동작 |
| --- | --- |
| 인사 표시 | users에서 name, growth_cycles에서 currentWeek (coaching_start_at 기준 계산) |
| 오늘 체크 토글 | daily_checks 즉시 업데이트 |
| 과거 일자 체크 | 7일 이내 과거만 수정 가능, 그 이전은 잠금 |
| 미래 일자 체크 | 차단 |
| 주간 진행률 | daily_checks 기반 자동 계산 |
...
- growth_cycles (active cycle)
- action_items (현재 사이클의 selected items)
- daily_checks (오늘의 액션 완료 여부)
- weekly_memos (이번 주)
- history_records (이전 주차 요약, 알림 카드 노출 판단)
...
| 이벤트 | 속성 |
| --- | --- |
| home_view | week_index, today_action_done |
| daily_action_toggled | week_index, day_index, new_state, source=home |
| home_card_clicked | card_type |
| timeline_week_clicked | week_index, status |
