...
| 다음 화면 | NEW01(미인증) / 03~10(온보딩 진행 중) / 11(active) / NEW03(완료) |
...
| ONBOARDING / ACTIVE / COMPLETED | 본인 상태 화면으로 자동 리다이렉트 |
...
- **1단계**: user.email_confirmed_at 확인 → NULL이면 NEW01 이메일 인증으로
- **2단계**: growth_cycles.status 확인
  -    · 없음 또는 abandoned → 마지막 온보딩 단계 다음 화면 또는 03
  -    · active → 11 홈
...
| 사용자 테이블 | auth.users + user_profiles 자동 생성 |
| 저장 항목 (가입 시) | name, email, password(hash), birthdate, consent_privacy/terms/marketing 및 각 timestamptz, policy_version |
