...
| 진입 경로 | 01 랜딩의 Primary CTA("동의하고 시작하기") → 회원가입 탭 / 직접 URL / 이메일 인증 링크 클릭 |
...
| 요소 | 설명 |
| --- | --- |
| 봉투 SVG 아이콘 | 파란 원형 배경 (accent-light, #EEF2FF) + 이메일 아이콘 |
| 타이틀 | "이메일을 확인해주세요" |
...
5. 위 조건 모두 해당 없음 → /onboarding/profile (온보딩 시작)
```
...
| 버전 | 날짜 | 변경 내용 |
| --- | --- | --- |
| v1.4 | 2026-05-12 | 구현 코드 기준 정렬: **(1) 진입 경로** — 01 랜딩 v1.5에서 Secondary CTA("이미 계정이 있어요") 제거됨에 따라 해당 진입 경로 삭제 / **(2) verify-email 아이콘 배경색** — "보라색 원형 배경" → "파란 원형 배경(accent-light, #EEF2FF)" / **(3) 라우팅 경로** — 온보딩 시작 경로 `/basic-info` → `/onboarding/profile` |
