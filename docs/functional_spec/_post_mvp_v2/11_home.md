...
| 완료 토글 | 미완료: "오늘 했나요? 탭해서 체크 👆" / 완료: "🎉 오늘 완료했어요!" |
| 완료 토글 서브 | 미완료: "강점 「{strength}」을 발휘하는 시간 ✨" / 완료: "잘했어요! 한 번의 체크가 다음 주를 만들어요 💪" |
| 7일 그리드 | 월~일 7칸. 완료 = done(accent-soft 배경 + accent 보더), 오늘 = today(흰색 + accent 보더 1.5px), 미래 = future(opacity .4, 클릭 불가) |

인터랙션:
- 토글 탭 → 오늘 완료 체크/해제 (`action_completions` INSERT/DELETE)
...
홈(active) / 회고 / 프로필 — 3탭 고정. sticky bottom.
