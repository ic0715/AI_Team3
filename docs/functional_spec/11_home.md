...
### 3.1 상단 바 `v1.4 수정 — 헤더 프로필 아이콘 제거 확정`

- 좌: CareerPT 로고
- 우: 알림 아이콘 (미읽음 시 빨간 dot badge) → (v2 구현)

> **15 프로필 진입은 하단 탭바로만** (00_flow.md v1.7 결정사항 정렬). 기존 v1.2의 "헤더 프로필 아이콘 → 15 프로필" 진입 경로는 제거됨.
...
| 버전 | 날짜 | 변경 내용 |
| --- | --- | --- |
| v1.4 | 2026-05-09 | 00_flow.md v1.7(프로토타입 정합성) 정렬: **3.1 상단 바** — 헤더 [프로필 아이콘] → 15 진입 경로 제거 확정. 15 프로필은 **하단 탭바로만 진입**. 기존 v1.2의 "프로토타입 수정 필요" 권고 주석은 무효(반대 결정으로 전환)되어 제거. |
