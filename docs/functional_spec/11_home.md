...
- 현재 시즌 테마 (career_focus.theme_title, status=active)
- 진행 주차 표시 (예: "3 / 12 주")
- 다음 회고 코칭 일정 ("이번 주 일요일 저녁")
- 12주 dot indicator (완료/현재/예정 색상 구분)
- 비고: career_focus는 selected_direction 기반으로 10 액션 선택 시 자동 생성됨

### 3.4 오늘의 액션 카드

- "오늘의 액션" 라벨 + 요일
- 이번 주 진행률 (예: "3/7 완료")
- 액션 텍스트 (선택한 weekly action)
- 체크 토글 버튼 (오늘 완료 여부)
...
| 기능 | 동작 |
| --- | --- |
| 인사 표시 | users에서 name, users.coaching_start_at 기준으로 currentWeek 자동 계산 |
| 오늘 체크 토글 | weekly_actions.daily_done JSONB 즉시 업데이트 |
| 과거 일자 체크 | 7일 이내 과거만 수정 가능, 그 이전은 잠금 |
| 미래 일자 체크 | 차단 |
| 주간 진행률 | weekly_actions.daily_done 기반 자동 계산 |
| 타임라인 클릭 | done → 해당 주차 회고/기록 보기, future → 미리보기 또는 비활성 |
| 회고 카드 클릭 | 12 회고 화면 |
| 알림 카드 클릭 | 해당 액션 화면 |
| 12주 완료 시 | 다음 진입 시 NEW03으로 자동 라우팅 |

## 5. 데이터

- users (name, coaching_start_at)
- career_focus (현재 시즌 테마, status=active)
- career_results (selected_direction 참조)
- weekly_actions (현재 주차 액션 + daily_done)
- action_items (현재 사이클의 selected items)
- weekday_memos (이번 주 평일 메모)
- insight_history (이전 주차 요약, 알림 카드 노출 판단)
- 주차 계산: coaching_start_at 기준

## 6. 예외 처리

| 상황 | 처리 |
| --- | --- |
| coaching_start_at 없음 | 10 액션 아이템으로 리디렉션 |
| weekly_plan 누락 | 자동 생성 시도 → 실패 시 안내 |
