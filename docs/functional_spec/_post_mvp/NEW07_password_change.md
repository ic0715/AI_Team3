...
| 다음 화면 | 11 홈 (변경 완료 시) / 15 프로필 (취소 시) |
...
| 변경 클릭 | 클라이언트 검증 → 재인증 → `supabase.auth.updateUser({ password })` 호출 → 성공 토스트 → **11 홈으로 자동 이동** (보안성 강조 + 변경 직후 다음 행동 시작 용이성) | 로딩 상태 표시 |
...
5. **11 홈으로 자동 이동** (15 프로필 복귀 X — 보안성 강조 + 변경 직후 다음 행동을 자연스럽게 시작, 00_flow.md v1.7 결정사항)
...
| 버전 | 날짜 | 변경 내용 |
| --- | --- | --- |
| v1.4 | 2026-05-09 | 00_flow.md v1.7(프로토타입 정합성) 정렬: **변경 성공 시 이동 화면 변경** — "15 프로필 복귀" → "**11 홈으로 자동 이동**" (보안성 강조 + 변경 직후 다음 행동 시작 용이성). 1번 화면 개요 다음 화면, 4번 기능 표 변경 클릭 행, 5번 변경 성공 후 동작 5번 항목 — 총 3곳 일괄 정렬. 취소 시는 15 프로필 복귀 유지. |
