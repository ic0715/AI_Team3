...
| 15 | 프로필 | MAINTAIN | 강점 요약·기본정보 인라인 수정·알림 설정·로그아웃 |
...
15 프로필
  ...
  ├─ [정보 수정] ─→ 15 화면 내 인라인 편집 모드 (별도 화면 이동 없음)
  ├─ [로그아웃] ─→ 01
  └─ [회원 탈퇴] ─→ 01
...
| 15 | 정보 수정 | 15 화면 내 인라인 편집 모드 (별도 화면 이동 없음, 저장 시 토스트) |
...
- **15 정보 수정**: ✅ 화면 내 인라인 편집 모드로 처리 (별도 03 진입 X)
  - "수정" 버튼 토글 → 폼 필드로 전환 → [저장]/[취소] 버튼 노출 → 저장 시 토스트 + 읽기 모드 복귀
  - 별도 페이지 이동 없이 한 화면에서 컨텍스트 유지, 03(기본정보 입력 화면)은 신규 가입 흐름 전용으로 단순화
