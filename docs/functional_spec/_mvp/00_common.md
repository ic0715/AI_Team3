...
| 커리어 방향 재설정 (15에서 진입) | 07로 진입 | 진입 경로 미구현 |
...
| 주요 UI | 브랜드 헤더, 코칭 원칙 카드 3개, "동의하고 시작하기 →" 단일 CTA (Secondary CTA "이미 계정이 있어요" 제거됨, v1.11) |
...
— 문서 끝 | CareerPT Product Spec v1.12 (NEW 프로토타입 v1 정합성 정렬: 04 강점 선택 재정의 + 갤럽 PDF 업로드 spec 폐지 + NEW02 커리어 방향 설정 완료 + 소셜 로그인 spec 제외 + §5.6 MVP 스코프 신규)
