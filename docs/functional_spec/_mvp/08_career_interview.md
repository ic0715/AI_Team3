...
### 3.6 분석 대기 화면 (loading overlay) `v2.1 사용자 인사이트 3 대응`

Path A/B 종료 감지 후 finalize API 호출 동안 노출되는 풀스크린 오버레이.

| 요소 | 카피 / 동작 |
| --- | --- |
| 아이콘 | 🔍 (이모지 36px) |
| 메인 카피 (16px / 굵게) | **"커리어 방향을 분석 중이에요"** |
| 시간 안내 (13px) | **"약 20초 정도 걸려요"** |
| 변동 안내 (12px, 보조 색상) | **"상황에 따라 조금 더 걸릴 수 있어요"** |
| 진행 표시 | LoadingDots 컴포넌트 |
| 사용자 인터랙션 | 차단 (오버레이 전체) |

**변경 배경 (사용자 조사 인사이트 3)**:
- 이전 카피 "약 5~12초 정도 걸려요" 실측 12초 이상 발생. **끝을 알 수 없는 침묵**이 "혹시 멈춘 건가?" 신호로 읽혀 신뢰 손실.
- 대응 원칙: (1) 실측 기준 시간 현실화, (2) 변동 가능성 사전 안내로 사용자가 길어져도 정상으로 인식하게 함.

> 코드 구현 참조: `web/app/onboarding/career-interview/page.tsx` (isFinalizing 분기).
