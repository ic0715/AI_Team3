...
2. 하단에 버튼 영역 노출:
   ...
   - Secondary: "💬 인터뷰 더하기" → 클릭 시 **인터뷰 더하기 모드** 진입 (아래 상태 표 참조)

**인터뷰 더하기 모드 동작 원칙 (v1.4 추가):**

- "💬 인터뷰 더하기" 클릭 시 입력창을 단순히 재노출만 하면 안 됨. **반드시 입력창 enable + focus 처리**해야 함 (이전 `proceedC` 호출에서 disabled 상태로 남아있을 수 있음).
- 추가 입력 모드에서는 코어 질문 흐름이 종료된 상태이므로 매 메시지 전송 시 새 코어 질문을 던지지 않고 짧은 ack로 응답한다.
- 추가 모드 진입 후에는 **"💬 인터뷰 더하기" 버튼은 숨김**(이미 더하는 중이므로 중복). "진단 완료하기" 버튼만 유지하여 사용자가 충분히 나누었다 싶을 때 마무리할 수 있도록 한다.

| 상태 | 조건 | UI |
| --- | --- | --- |
| 진행 중 | 코어 질문 카운터 < 6 | 입력창 활성, 버튼 영역 숨김 |
| 완료 감지 | 코어 질문 카운터 = 6 | 입력창 숨김, 버튼 영역 노출 (Primary "진단 완료하기" + Outline "💬 인터뷰 더하기") |
| 인터뷰 더하기 모드 진입 | "💬 인터뷰 더하기" 클릭 | 입력창 재노출 + **enable** + focus, "💬 인터뷰 더하기" 버튼 숨김, 짧은 안내 멘트 노출 |
| 추가 입력 후 | 사용자가 추가 메시지 전송 | 짧은 ack 응답 (예: "좋은 이야기 더 들었어요 ✨"), 입력창 enabled 유지, "진단 완료하기" 버튼만 노출 (계속 자유롭게 추가 가능) |
