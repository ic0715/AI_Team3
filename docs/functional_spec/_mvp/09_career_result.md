...
### 3.6 분석 대기 상태 (SkeletonCards) `v1.6 사용자 인사이트 3 대응`

화면 진입 시 Step2 AI 개인화 호출 동안 노출되는 대기 UI. 단순 정적 스켈레톤이 아니라 **분석이 진행 중임을 시각·언어로 명확히 전달**.

| 요소 | 사양 |
| --- | --- |
| 상단 상태 배너 | 스피너(accent 16px) + 메시지 텍스트, `var(--accent-tint)` 배경 + accent 보더 |
| 메시지 — 0~7초 | **"역량을 찾는 중이에요..."** |
| 메시지 — 7초 경과 후 | **"거의 다 됐어요"** (자동 전환) |
| 메시지 전환 | `aria-live="polite"`로 스크린리더에 알림 |
| 카드 영역 | 회색 카드 5개. **pulse 애니메이션** (opacity 0.45 ↔ 0.75 반복, 카드별 0.12s stagger) |
| 사용자 인터랙션 | 차단 (카드 `pointer-events: none`, `aria-busy="true"`) |

**변경 배경 (사용자 조사 인사이트 3)**:
- 이전 정적 스켈레톤(`opacity: 0.55`만)은 ~10초간 침묵하며 "에러난 건가?" 인상.
- 대응 원칙: (1) 상태 텍스트 노출로 "분석 진행 중" 명확화, (2) 시간 경과 시 메시지 전환으로 "거의 끝났다" 신호, (3) 카드 pulse로 정적 인상 제거.

> 코드 구현 참조: `web/app/onboarding/career-result/page.tsx` (`SkeletonCards` 컴포넌트).
...
| 매칭/개인화 진행 중(로딩) | **§3.6 분석 대기 상태(SkeletonCards) 노출** — 상태 배너(7초 후 메시지 전환) + pulse 애니메이션. 사용자 인터랙션 차단 |
...
| 버전 | 날짜 | 변경 내용 |
| --- | --- | --- |
| v1.6 | 2026-05-26 | **3.6 분석 대기 상태(SkeletonCards) 신규** — 사용자 조사 인사이트 3 대응. 기존 정적 스켈레톤(`opacity: 0.55`)이 ~10초 침묵으로 "에러난 건가?" 인상 → 상단에 스피너+상태 메시지 배너 추가("역량을 찾는 중이에요…" → 7초 후 "거의 다 됐어요" 자동 전환, `aria-live="polite"`), 카드 pulse 애니메이션(opacity 0.45↔0.75, 카드별 0.12s stagger)으로 정적 인상 제거. §6 예외 처리 표의 "매칭/개인화 진행 중" 행을 §3.6 참조로 연결. 코드 구현 참조: `web/app/onboarding/career-result/page.tsx` (`SkeletonCards`). |
