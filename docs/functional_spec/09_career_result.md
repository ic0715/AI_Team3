...
> AI가 도출한 커리어 방향 후보 3~5가지를 제시하고, 유저가 원하는 방향을 선택하게 함.
...
### 3.4 목표 후보 카드 (3~5개) `v1.2 수정 — fit badge 추가`

> ⚠️ **schema 구조**: 커리어 방향은 AI 추천 `goal_title` 후보 3~5개로 표시.
...
| fit badge | AI가 카드별로 1개 선택 (아래 허용값 참조) |
...
| 텍스트 | 스타일 | 사용 조건 |
| --- | --- | --- |
| "강점 연계 높음" | green badge | 강점과 직접 연결되는 방향 |
| "성장 잠재력 높음" | blue badge | 현재 약하지만 발전 가능성이 높은 방향 |
| "추천" | accent badge | AI가 가장 적합하다고 판단한 1개 카드 |
| (없음) | — | 일반 옵션 |

> ⚠️ "추천" badge는 카드 전체에서 최대 1개만 사용. 나머지는 "강점 연계 높음" 또는 "성장 잠재력 높음" 중 선택하거나 badge 없이 표시.
...
>
> ⚠️ **프로토타입 수정 필요**: 프로토타입 v6은 08 완료 후 카드를 즉시 표시(1단계)하는 방식으로 구현되어 있음. 다음 버전에서 "목표 추천받기" 버튼 추가 및 2단계 흐름으로 수정 필요.
...
| 선택 해제 | 동일 카드 재클릭 시 |
| "액션 아이템 받기" 클릭 | `goals` INSERT (`goal_category`, `goal_title`, `career_interview_id`) → 10 이동 |
...
| `career_result_confirmed` | `selected_index`, `goal_category` |
