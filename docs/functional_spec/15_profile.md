...
| 진입 경로 | 탭바 "프로필" |
| 다음 화면 | 06(강점 상세) / 04(강점 재분석) / 07(커리어 재설정) / NEW04(알림) / NEW07(비밀번호) / 회원 탈퇴 |
...
### 3.5 기본 정보 섹션 `v1.3 수정 — 인라인 편집 모드로 확정`

- 표시: 이름 / 나이대 / 직군 / 경력 / 현재 커리어 고민
- "정보 수정" 버튼 → **15 화면 내 인라인 편집 모드** (별도 화면 이동 없음)
  - "수정" 버튼 토글 → 폼 필드로 전환 → [저장]/[취소] 버튼 노출
  - 저장 시 변경된 필드만 `profiles` 테이블 UPDATE → 토스트 "정보가 수정되었어요" + 읽기 모드 복귀
  - 취소 시 변경사항 폐기 → 읽기 모드 복귀
...
| 정보 수정 | **15 화면 내 인라인 편집 모드** — "수정" 버튼 토글 → 폼 필드 전환 → [저장]/[취소] 버튼 노출 → 저장 시 변경된 필드만 `profiles` UPDATE + 토스트 "정보가 수정되었어요" + 읽기 모드 복귀 / 취소 시 변경사항 폐기 후 읽기 모드 복귀 (별도 화면 이동 없음) |
