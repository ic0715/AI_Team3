...
- 이메일
- 배지: 직무
- ~~연속 활동 streak 배지~~: v1 제외 (후순위 — 갱신 정책 미확정)
...
### 3.5 기본 정보 섹션 `v1.4 수정 — 03 수정 모드로 회귀 (v1.3 인라인 편집 결정 되돌림)`

- 표시: 이름 / 나이대 / 직군 / 경력 / 현재 커리어 고민
- "정보 수정" 버튼 → **03 (수정 모드)로 이동** (별도 화면 이동, v1.6 방식 환원)
  - 03 화면에서 기존 값 prefill → 변경된 필드만 `profiles` 테이블 UPDATE → 15 복귀 + 토스트 "정보가 수정되었어요"
...
| 버전 | 날짜 | 변경 내용 |
| --- | --- | --- |
| v1.5 | 2026-05-09 | 팀 결정사항 반영: **streak 배지 v1 제외** — 프로필 헤더 "연속 활동 streak" 배지 후순위로 제외 (갱신 정책 미확정, 앱 출시 후 검토) |
| v1.4 | 2026-05-09 | 00_flow.md v1.8 / 00_common.md v1.9 정합성 정렬: **정보 수정 흐름 회귀** — v1.3에서 "15 화면 내 인라인 편집 모드"로 변경했던 결정을 **되돌려 "03 (수정 모드) → 15 복귀" v1.6 방식으로 환원**. 1번 화면 개요 다음 화면 표에 03(정보 수정) 추가, 3.5 기본 정보 섹션 + 4번 기능 표 일괄 정렬. v1.3에서 제거됐던 "11 헤더 프로필 아이콘" 진입 경로는 v1.4에서도 그대로 유지(15는 하단 탭바로만 진입). |
| v1.3 | 2026-05-09 | 00_flow.md v1.7(프로토타입 정합성) 정렬: **(1) 진입 경로 정리** — 1번 화면 개요·2번 진입 조건의 "11 헤더 프로필 아이콘" 항목 제거. 15는 **하단 탭바로만 진입**. 1번 표 "다음 화면"에서 "03(정보 수정)" 제거. **(2) 정보 수정 흐름 변경** — 3.5 기본 정보 섹션 + 4번 기능 표 — "[정보 수정] 버튼 → 03 수정 모드로 이동" → "**15 화면 내 인라인 편집 모드**(별도 화면 이동 없음)". "수정" 토글 → 폼 필드 전환 → [저장]/[취소] 버튼 노출 → 저장 시 변경 필드만 `profiles` UPDATE + 토스트 + 읽기 모드 복귀. |
