...
> ### ⚙️ 구현 현황 (2026-06, 온보딩 액션 화면 — C 생성 모델)
>
> 이 명세의 "시드 풀에서 선택" 모델은 **C(생성)+검증 게이트**로 개정됨(고정 풀의 커버리지 천장 제거). 시드 풀은 **폴백/보충 안전망**으로만 남는다.
>
> - **생성(①)**: 사용자 인터뷰·강점·직무에서 액션 후보 N개(현재 7개)를 **직접 생성**. 각 후보는 Top5 강점 하나를 실제 발휘(`strength_link`)하고 역량에 직결되어야 함. `competency_action_map.md`는 인용 대상이 아니라 "행동의 결" 참고.
> - **검증 게이트(②)**: 별도 LLM 콜로 후보별 통과 판정 — 역량적합·**강점연계**·ICF·**정서위기 안전(🔴/🟠/🟡, 비협상)**·형식. 통과분만 노출(최대 5). 강점연계는 생성 단계에서도 1차 강제(Top5 밖 strength_link 폐기).
> - **폴백**: 게이트 통과분이 5 미만이면 검증된 풀(`seeds.ts`)로 보충, 생성/게이트 실패(파싱오류 등) 시 풀로 전체 폴백. 게이트를 통과 못 한 생성분은 **절대 노출 안 됨**.
> - **`source_seed_id`**: 생성분 `null`(시드 인용 아님), 폴백분 시드 안정 id(`T-1-junior-2` 등). `strength_link`는 생성분=발휘 강점, 폴백분=Top1 대체.
> - **시드 풀 구성**(폴백용): `competency_action_map.md` 전사(12역량×주니어3+시니어3) + 레거시 extras = 역량당 6~11개. `level`·`effort` 메타 보유.
> - **judge(게이트) 벤더**: 런타임은 동일 벤더(Anthropic) critic 콜(자기채점 한계 인지). 진짜 크로스벤더 검증은 오프라인 eval(`web/lib/actionItems/actionEval.eval.test.ts`, OpenAI judge).
> - **미구현**: 12주 주차별 순환·dedup. 온보딩 1주차 단발. 주간 회고는 별도(`reflect/coach/page.tsx`).
