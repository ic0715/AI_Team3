 ...
 * ([{slot, personalizedText}, ...])을 결정적 매칭 결과(matchedSlots)에
 * 머지해 personalizedText를 채운다. 이 검증/머지 로직만 따로 떼어 단위 테스트한다.
 *
 * 두 가지 방어 지점:
 *  1. `validatePersonalizedResponse` — LLM이 기대 개수(매칭 슬롯 수)만큼 배열을 반환하지 않으면 던진다.
 *     (배열 아님 / 길이 != expectedCount). 슬롯 수는 매칭 결과 길이로 정해지므로 고정값에 의존하지 않는다.
...
/**
 * LLM이 반환한 파싱 결과가 기대 개수(expectedCount)짜리 배열인지 검증.
 * expectedCount는 결정적 매칭 슬롯 수(matchedSlots.length)와 일치해야 한다.
 * 아니면 개수를 노출한 메시지로 throw.
 *
...
export function validatePersonalizedResponse(
  ...
  if (!Array.isArray(parsed) || parsed.length !== expectedCount) {
    throw new Error(
      `LLM이 ${expectedCount}개 슬롯을 반환하지 않음: ${(parsed as { length?: number })?.length ?? 0}개`,
    );
  }
}

/**
 * matchedSlots(결정적 매칭 결과)에 parsed의 personalizedText를 머지.
