...
export default function DevPageClient({ steps }: { steps: Step[] }) {
    ...
    <div className="min-h-screen bg-[#e8eaee] flex justify-center items-start py-10 px-4">
      <div className="w-full max-w-md flex flex-col gap-5">
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm flex flex-col gap-3">
          ...
          {user ? (
            <div className="flex flex-col gap-2">
              ...
              <button onClick={handleLogout} className="text-sm text-red-500 hover:text-red-700 text-left">
                로그아웃
              </button>
            </div>
          ) : (
            <div className="flex flex-col gap-2">
              <input type="email" placeholder="테스트 이메일" value={email}
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm flex flex-col gap-3">
          ...
          <button onClick={handleSeed} disabled={loading || !user}
            className="w-full py-2 bg-blue-600 text-white rounded-xl font-bold text-sm disabled:opacity-50 hover:bg-blue-700 transition-colors">
            {loading ? "처리 중..." : "🌱 테스트 데이터 심기"}
          </button>
          <button onClick={handleReset} disabled={loading || !user}
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2">
            {steps.map((step, i) => (
              <button key={step.id} onClick={() => router.push(step.path)}
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm flex flex-col gap-2">
          ...
          <pre className="bg-gray-900 text-green-400 rounded-xl p-3 text-xs leading-relaxed overflow-x-auto">{`git pull origin main
cd web && npm install
npm run dev`}</pre>
          <p className="text-xs text-gray-400 leading-relaxed">
